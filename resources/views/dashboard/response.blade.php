    ...
    <form action="{{route('response.update', $reportId)}}" method="POST" style="width: 500px; margin: 50px auto; display:block">
        ...
        <div class="input-card">
            ...
            <select name="status" id="status">
                ...
                <option value="ditolak" {{$report['status'] == 'ditolak' ? 'seleceted' : ''}}>ditolak</option>
                <option value="proses" {{$report['status'] == 'proses' ? 'seleceted' : ''}}>proses</option>
                <option value="diterima" {{$report['status'] == 'diterima' ? 'seleceted' : ''}}>diterima</option>
            </select>
            @else
            <select name="status" id="status">
                <option selected hidden disabled> Pilih status</option>
